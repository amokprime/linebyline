...
Run via the project `agent-tst` wrapper — see `AGENTS.md` for the wrapper's allowed arguments. It is NOT on the agent's PATH; invoke it by full path as `~/.local/bin/agent-tst`. Do not invoke `npx playwright` or raw `ssh` directly; the wrapper handles the SSH test infrastructure and master-key separation.

```sh
~/.local/bin/agent-tst -g <pattern>            # scoped run (preferred)
~/.local/bin/agent-tst tests/foo.spec.js       # single file
~/.local/bin/agent-tst --list                  # enumerate tests
~/.local/bin/agent-tst --project=chromium      # browser project override
```

A full suite takes about 7 minutes and exceeds the synchronous tool timeout. Keep runs scoped unless a full pass is genuinely required.
...
- **`typing-debounce-1`** (intervals.spec.js, chromium) — 1ms debounce + Playwright's <1ms keypresses → all letters land in one snapshot. Add `waitForTimeout(20)` between keypresses so the debounce fires separately for each. 20ms is well above the ~16ms rF interval and ~10ms clock-tick resolution, ensuring separate snapshots even on a contended CI runner. (5ms was sufficient locally but flaky on CI where the browser event loop is under contention.)
