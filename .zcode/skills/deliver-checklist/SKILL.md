...
2. **When the change set is durable, extract it to a skill.** Use the test: "would a fresh model in a new session need this?" — if yes, it belongs in a skill. Skills hold reusable patterns, procedures, and reference catalogs; harness memory holds app-specific history and one-off facts. A memory entry that starts explaining *how* instead of *what happened* is a skill gap — extract the general principle into the skill and prune the entry to a one-line reference.
3. **Update skills in-place, not by appending.** When the change shifts the architecture a skill documents (helper extraction, new restricted key, new SonarQube rule, new Playwright helper), update the skill in-place. Don't add "Turn N: extracted helper X" appendices — those are commit-log noise. Re-read the affected skill before editing to avoid drift. Prefer structural references over line numbers ("find it by grepping `// ──` markers"); when a line number is unavoidable, label it as a point-in-time reference.
4. **Cross-skill consistency.** When updating one skill, check whether an existing skill already covers the pattern. If so, update both. Before delivering a new or heavily revised skill, do the cheap static review: does the description trigger accurately (and not over-trigger), can a fresh model follow the body without session context, does every section earn its token cost, and does anything contradict another skill?
...
- **Session transcript capture** — the harness session system keeps transcripts; the user archives the important ones under `archive/`. Don't recreate a hand-written transcript system.
