...
- **Tranche 2 — Playwright retarget to Vite preview** (pre-cutover, on `staging`) — the full Playwright + Codegen + UI-mode toolchain currently hardcodes `docs/index.html` as the target. This tranche adds a `LBL_VITE_TARGET` env-var branch to every layer so the Vite build can be tested without removing the monolith target. **Three files change**:
...
- **Tranche 3 — Local-test checkpoint** (pre-cutover, the gate) — **this is the point where the user tests the Vite build locally before merging to `main`**. No code changes in this tranche — it's a verification gate using the toolchain wired in Tranche 2. The user runs four test categories and pastes results before proceeding to Tranche 4 (logic.spec.js consolidation) or Tranche 5 (cutover):
