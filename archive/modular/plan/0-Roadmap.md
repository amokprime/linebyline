...
Each tranche ships its verification + cutover step + Vitest/Playwright specs. Tranches 1–4 are pre-cutover (still on `staging`, monolith still live); tranche 5 is the cutover itself; tranches 6–8 are post-cutover cleanup. **All tranches require the user to run the full Playwright suite locally via `tst`** — the sandbox cannot run Playwright (snapshot data overwhelms context). The agent writes test code + verification scripts; the user runs them and pastes results. **Tranche 3 (local-test checkpoint) is the gate** — the user manually verifies the Vite build in both Playwright Codegen and a real browser before the cutover.
...
- **Tranche 3 — Local-test checkpoint** (pre-cutover, the gate) — **Status: in progress (Sep 2026, session 11).** Root cause found for session 10's "patches didn't work": `deploy.sh` was missing `npm run build` — `vite preview` served stale `dist/`. Fix: `deploy.sh` now rebuilds `dist/` after Vitest. Also fixed `tst-vite-log` fish function (grep `\d` → `[0-9]`, simplified pattern). Sandbox-verified 17/19 sample tests pass (chromium). See `tests/FAILURES.md` for full status. No code changes in this tranche — it's a verification gate using the toolchain wired in Tranche 2. The user runs four test categories and pastes results before proceeding to Tranche 4 (logic.spec.js consolidation) or Tranche 5 (cutover):
...
- **Tranche 4 — `tests/logic.spec.js` consolidation** (pre-cutover, sandbox-safe) — the Phase E cleanup reminder (below) lists 4 functions unique to `logic.spec.js` (`_peelLastParen`, `batchSplitParens`, `_findNextTimestampMs`, `_assignInterpolatedTs`) and 10 functions shadowed between `logic.spec.js` (Playwright, pre-refactor) and the Vitest unit suite. Tranche 5 of Phase D already ported the 4 unique functions to `src/utils/timestampSync.ts` with Vitest coverage. This tranche deletes the corresponding `logic.spec.js` sections once Tranche 3's local-test checkpoint is green — the Vitest specs replace them. **Order**: (a) confirm Tranche 3 checkpoint passed (all four test categories green); (b) delete the 4 unique-to-`logic.spec.js` sections (L166–250: `_peelLastParen` + `batchSplitParens`; L586–end: `_findNextTimestampMs` + `_assignInterpolatedTs`); (c) delete the 10 shadowed sections (LRC parsing, key normalization, hotkey restriction, `mergeLrcMeta`/`ensureReTagDefault`, `cleanPaste`/`cleanGenius`, `collapseBlanks` — the Vitest versions are the parity bridge now); (d) if `logic.spec.js` becomes empty, delete the file entirely. **Risk**: if a Vitest spec is weaker than the Playwright spec it replaces (e.g. doesn't cover an edge case the Playwright version had), the consolidation loses coverage. Audit each section before deletion — if the Vitest spec is missing an edge case, port it first. **No new specs** — this tranche deletes specs, doesn't add them. **Files touched**: `tests/logic.spec.js` (sections deleted or file removed).

- **Tranche 5 — Cutover: Pages source flip + deploy.yml trigger** (the cutover itself, on `main`) — this is the single moment the live site changes. **Pre-conditions**: Tranches 1–4 done; Tranche 3 local-test checkpoint passed; CodeQL + SonarCloud gates green on the PR. **Steps** (executed by the user, the agent writes the deploy.yml + workflow changes):
...
- **`tests/logic.spec.js` is the parity bridge.** Don't delete it until Phase E Tranche 4 confirms the Vitest replacements are green under the Vite-target Playwright run (Tranche 3 checkpoint). Deleting early loses the bridge; deleting late leaves a stale file that confuses fresh sessions.
