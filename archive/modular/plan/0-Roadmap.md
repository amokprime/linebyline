...
- **Tranche 4.5 — Regression coverage + MANUAL.md automation** (pre-cutover, sandbox-safe) — dogfood the localhost Vite build (`srv`) to discover remaining feature regressions, then automate as many `tests/MANUAL.md` items as reasonably possible. Each regression found during Phase E pre-cutover testing revealed a test coverage gap — the missing highlighter/border wasn't caught by any Playwright test (screenshot tests were dropped), the nonfunctional hotkeys weren't caught (Playwright's synthetic events bypass browser-level interception), the refresh-persistence wasn't caught (no Playwright test reloads + checks empty state). This tranche closes those gaps before the cutover.
    ...
    - **Playwright test additions** (add back coverage that was dropped during the modular port):
        ...
        - Refresh behavior: `page.reload()` then verify `#main-textarea` is empty (no lyrics persisted). Verify audio is not loaded (`#time-dur` shows `0:00`).
        - Seek slider drag: verify audio pauses during drag (not staccato). Use `page.mouse` to simulate mousedown + mousemove + mouseup.
        - Settings ⌨ icon: verify clicking it returns focus to the search field (not stuck on the button).
        - `Ctrl+\` reset defaults: verify it opens Settings + shows the Yes/No confirm dialog (not just opens Settings).
        - `Ctrl+Shift+~` toggle panel: verify it collapses/expands the NOW PLAYING panel (replaces `` Ctrl+` `` which Chromium-based browsers intercept).
    - **Screenshot vs ARIA tradeoff**: the old screenshot tests caught the missing highlighter but were dropped due to font-fragility (Fedora vs Ubuntu viewport mismatch). Consider: (a) adding back screenshot assertions with `maxDiffPixelRatio` tolerance for font rendering, OR (b) refactoring to ARIA-based assertions (e.g. `toMatchAriaSnapshot` for structure + computed-style checks for visual properties). ARIA is more robust but can't verify color/border visibility — computed-style checks fill that gap.
    - **Real browser automation** (if Playwright can't cover): investigate Playwright's `launch({ head: false })` for real-browser tests that bypass synthetic event limitations. Covers hotkey interception (Ctrl combos), page refresh, Genius paste (real clipboard), 10k line import (real file picker). May require a separate test runner or `playwright.config.js` project.
    - **Scaffolding updates** (if time permits):
        ...
        - Try zip + symlink instead of Repomix: zip the actual repo files (preserving structure) and symlink them into the sandbox. Avoids Repomix's XML wrapping + file-size limits. Covered in `scratch.md` draft.
