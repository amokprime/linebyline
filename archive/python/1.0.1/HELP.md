...
### Playback controls
- **Seek increment** (default 5s) by scrolling over the seek bar, using `Ctrl+A` or `Ctrl+D`, or clicking the left/right media buttons
- Play/pause with `Space` in **Hotkey mode** or `Ctrl+Space` in any mode
- ⚙️📝**Speed ratio** (`Ctrl+1`, `Ctrl+1`, `Ctrl+3`, up/down arrows) multiplies against current speed
- **Instant Replay** with `R` (start), `Shift+R` (end), or configurable triggers
	...
	- **Seek offset** (up/down arrows) gives you more time to react
...
- Sync a line (`W`, `Enter`) to move to the next line
	...
	- Replay it again with `R` or move to the next line with `E`, `ArrowDown`, or `Click`.
...
- You can also mark individual lines as translations with `Ctrl+ArrowLeft`
