...
from PySide6.QtWidgets import (
    ...
    QToolBar, QMessageBox, QFileDialog, QComboBox, QLabel,
    QApplication,
)
from PySide6.QtCore import Qt, QTimer, QByteArray, QEvent
from PySide6.QtGui import QKeySequence, QAction, QIcon, QPixmap, QPainter, QColor
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        saved_font = self.cfg.get("editor_font", "Segoe UI")
        idx = next((i for i, (_, f) in enumerate(FONT_OPTIONS) if f == saved_font), 0)
        ...
        self._font_size_spin = _QSB()
        self._font_size_spin.setRange(8, 32)
        self._font_size_spin.setValue(self.cfg.get("editor_font_size", 14))
        self._font_size_spin.setFixedWidth(52)
        self._font_size_spin.setFixedHeight(26)
        self._font_size_spin.setAlignment(Qt.AlignmentFlag.AlignRight)
        self._font_size_spin.setStyleSheet(
            "QSpinBox { padding-right:12px; font-size:12px; }"
            "QSpinBox::up-button, QSpinBox::down-button { width:12px; }"
        )
        self._font_size_spin.editingFinished.connect(self._on_font_size_changed)
        ...
        # Install event filter so key events from editor reach keyPressEvent
        self.editor.installEventFilter(self)

        # App icon (inline SVG rasterised to 32×32)
        _set_app_icon(self)
    ...
    def _connect_signals(self) -> None:
        ...
        self.editor.open_requested.connect(self._do_import)
    ...
    def _restore_session(self) -> None:
        ...
        # Apply saved font
        from config import FONT_OPTIONS
        saved_font = self.cfg.get("editor_font", FONT_OPTIONS[0][1])
        self.editor.apply_font(saved_font)
        saved_size = self.cfg.get("editor_font_size", 14)
        self._font_size_spin.blockSignals(True)
        self._font_size_spin.setValue(saved_size)
        self._font_size_spin.blockSignals(False)
        self.editor.apply_font_size(saved_size)
    ...
    def _on_audio_loaded(self, path) -> None:
        ...
        # Reset Now Playing display unconditionally
        self.controls.update_title(path.stem, "Unknown Artist")
    ...
    def _on_font_changed(self, idx: int) -> None:
        from config import FONT_OPTIONS
        _, family = FONT_OPTIONS[idx]
        self.cfg["editor_font"] = family
        save_cfg(self.cfg)
        self.editor.apply_font(family)

    def _on_font_size_changed(self) -> None:
        size = self._font_size_spin.value()
        self.cfg["editor_font_size"] = size
        save_cfg(self.cfg)
    ...
    def eventFilter(self, obj, event) -> bool:
        """Forward key events from editor panel up to our keyPressEvent."""
    ...
    def keyPressEvent(self, e) -> None:
        """Handle navigation keys and Tab mode-toggle."""
        # Tab always toggles mode regardless of which widget has focus
        if e.key() == Qt.Key.Key_Tab and not e.modifiers():
            e.accept()
            self._dispatch_action("toggle_mode")
            return
...
QLabel { color: #e6edf3; }
QScrollBar:vertical { background: #161b22; width: 10px; border: none; }
...
/* Settings title bar and footer */
QWidget#audioBox { border: 1px solid #30363d; border-radius: 6px; }
"""
...
def _set_app_icon(window: "MainWindow") -> None:
    """Set the window/taskbar icon using the same SVG as the HTML favicon."""
    px = QPixmap(32, 32)
    ...
    # Yellow lines (right side)
    p.setPen(QPen(QColor("#ffff00"), 1.5))
    for y in (5, 12, 19):
        p.drawLine(QPointF(13, y), QPointF(21, y))
    # Green checkmarks (left side)
    p.setPen(QPen(QColor("#00ff00"), 1.5))
    p.drawPolyline([QPointF(3,17), QPointF(5,19), QPointF(9,15)])
    p.drawPolyline([QPointF(3, 7), QPointF(5, 9), QPointF(9, 5)])
    p.end()
    icon = QIcon(px)
