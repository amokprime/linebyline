...
- Name: sync-adjust.spec.js >> adjust-seek
- Location: tests\sync-adjust.spec.js:32:1
...
Locator: getByRole('spinbutton', { name: 'Seek offset (ms): shifts' })
Expected: "-500"
Timeout: 5000ms
      ...
      - region "Now playing" [ref=e51]:
        ...
        - generic [ref=e58]:
          ...
          - button "Seek forward" [ref=e71] [cursor=pointer]:
            - img [ref=e72]
        - generic [ref=e75]:
          - spinbutton "Seek offset in milliseconds" [ref=e76]: "-500"
          - generic [ref=e77]: ms
          - generic [ref=e78]:
...
```ts
  ...
  2   | 
  3   | test("sync-time", async ({ page, media }) => {
  4   |   await page
  5   |     .locator("#file-picker")
  6   |     .setInputFiles([media("audio.mp3"), media("synced_english.lrc")]);
  7   |   await page.keyboard.press("ArrowDown");
  8   |   await page.keyboard.press("w");
  9   |   await expect(page.getByText("[00:00.00] That smell")).toBeVisible();
  10  | });
  11  | 
  12  | test("sync-time-end", async ({ page, media }) => {
  13  |   await page
  14  |     .locator("#file-picker")
  15  |     .setInputFiles([media("audio.mp3"), media("synced_english.lrc")]);
  16  |   await page.keyboard.press("t");
  17  |   await expect(
  18  |     page.locator("div").filter({ hasText: /^\[00:00\.00\]$/ }),
  19  |   ).toBeVisible();
  20  | });
  21  | 
  22  | test("adjust-time", async ({ page, media }) => {
  23  |   await page
