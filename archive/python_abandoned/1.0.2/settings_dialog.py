...
class SettingsDialog(QDialog):
    ...
    def _build(self) -> None:
        ...
        # No hardcoded background — inherits theme palette
        tbar.setStyleSheet("border-bottom:1px solid palette(mid);")
        tbar_lay = QHBoxLayout(tbar)
        tbar_lay.setContentsMargins(12, 0, 8, 0)
        tbar_lay.setSpacing(8)

        # No QLabel("Settings") here — window title bar already shows it
        ...
        footer.setStyleSheet("border-top:1px solid palette(mid);")
    ...
    def _build_section_intervals(self) -> QWidget:
        ...
        for sp in (self._spin_tiny, self._spin_small, self._spin_medium, self._spin_large,
            ...
            sp.valueChanged.connect(lambda _: self._save())
...
class _HkRow(QWidget):
    ...
    def _clear(self) -> None:
        ...
        self._last_good = ""
        self._capture.setText("")
        # Move focus to the row widget itself (not next field in tab order)
