...
HK_SECTIONS: list[dict] = [
    {"label": "Menu",        "keys": ["open", "save", "reset", "theme_toggle", "settings", "help", "undo", "redo"]},
...
HK_LABELS: dict[str, str] = {
    ...
    "reset":              "Reset (clear song and lyrics)",
}

# ── Font name map (CSS name → Qt font family, Windows) ───────────────────────
FONT_OPTIONS: list[tuple[str, str]] = [
    ("System Sans",  "Segoe UI"),
...
def migrate_cfg(raw: dict) -> dict:
    ...
    # Add missing keys that postdate a saved config
    if not hk.get("reset"):
        hk["reset"] = "Ctrl+R"
