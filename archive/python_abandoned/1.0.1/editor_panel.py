...
class EditorPanel(QWidget):
    # ── Signals ───────────────────────────────────────────────────────────────
    text_changed        = Signal(str)          # main text changed (any source)
    title_changed       = Signal(str, str)     # (title, artist) updated
    line_counts_changed = Signal()             # recheck merge button state
    genius_pasted       = Signal(str)          # raw Genius text pasted (main field)
    paste_done          = Signal()             # after any paste
    line_play_requested = Signal(int)          # user clicked a line → play it
...
class _LrcLineWidget(QLabel):
    ...
    def _render(self) -> None:
        ...
        if self._is_active:
            ...
            bg = self._C_NORMAL_BG
            ts_col = self._C_TS
            weight = "normal"

        if end:
            color = self._C_END_TS
            self.setText(f'<span style="color:{color};font-style:italic;">{_esc(line)}</span>')
        elif ms is not None:
            ts_part = line[:10]
            rest = line[10:]
            self.setText(
