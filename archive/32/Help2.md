###### About
LineByLine is an opinionated web app for manually syncing song lyrics...line by line. 
License: [GNU GPLv3](https://www.gnu.org/licenses/gpl-3.0.en.html) 
###### Disclaimer & Credits
This help file, workflow design, and most UI decisions were written by me and reviewed with Claude Sonnet 4.6. Undo and redo icons use Lucide; others are markdown or stock. Themes are Claude's take on GitHub Dark / GitHub Light.
This app is 🚨🌈VIBECODED🌈🚨 — ⚠️USE AT YOUR OWN RISK🗣️🗣️🗣️
###### Features
- Remappable hotkey-driven workflow
- Batch offset the actual timestamps
- Genius lyrics and metadata extraction from raw paste
- Configurable **Instant Replay** with offset and triggers
- Merging translations without highlighting them in music players
###### Settings⚙️
- Default hotkey (`Ctrl+,`); changes apply immediately
- Search for a setting by name (`Tab`)
- Keyboard button (`Tab`) searches by hotkey
...
- Reloading or closing the tab resets song and lyrics but persists settings
###### Instant Replay and Seek offset
- **Seek increment** (default 5s) by scrolling over the seek bar or using `Ctrl+A` or `Ctrl+D`
- Trigger **Instant Replay** with `R` or with configurable triggers
	- Seek the start (`Click`, `Spacebar`) or end (`Shift+Spacebar`) of a line
...
- Sync a line (`W`, `Enter`) to move to the next line
	...
	- Replay it again with `R` or move to the next line with `E`, `Down`, or `Left Click`.
- Navigate with `Q`/`E` or `Up`/`Down` in **Hotkey mode**
	- ⚙️✅"Moving to previous line" replays on `Q`
	- ⚙️✅"Moving to next line" replays on `E`
- Adjust lines with **Controls** in **Offset time** mode
	- Select multiple lines with `Ctrl/Shift+Click` or `Shift+Up/Down`
...
- **Add**/**Hide**/**Merge** fields (`Ctrl+4`, `Ctrl+5`, `Ctrl+6`)
	- Paste or **Add lyrics** file
	- Parentheses mean "not main lyrics" and can be disabled
...
- Keystrokes within undo window (default 150 ms) are grouped into one undo step