...
# SonarQube Cloud Workflow — LineByLine
...
**Reading strategy:** Read the first instance of each unique rule to understand it; skip `why.md`/`how.md` for subsequent duplicates of the same rule. Extract line numbers and function names from `where.json` for each instance.
...
### for-of conversion (S4138) — CAUTION
Convert only when the loop index is **not used** for:
- Accumulation via index (e.g., `result[i] = ...`)
- Output assignment keyed to index
- Indexed mutation of a parallel array
- Any expression involving `i` other than `arr[i]`

When in doubt, skip and document as Won't Fix.

### replaceAll (S1321) — CAUTION
Convert only when the search value is a **fixed string**. Skip if the regex has quantifiers (`+`, `*`, `?`, `{n}`), character classes, or anchors — `replaceAll` with a regex argument behaves the same as `replace` with `/g`, which SonarQube already accepted.

### String.raw on regex literals (S6443) — ALMOST ALWAYS FALSE POSITIVE
SonarQube flags regex literals like `/\d+/` as needing `String.raw`. This is incorrect — `String.raw` applies to template literals, not regex literals. Mark as Won't Fix with rationale: "False positive: rule does not apply to regex literal syntax."

### Math.min/max ternaries (S4023) — CHECK CAREFULLY
Not every `a > b ? a : b` is a min/max replacement. If the ternary involves side effects, string coercion, or a non-numeric comparison, skip it.

### Negated condition (S3800) — CHECK FOR ELSE
Only invert the condition if there is a meaningful `else` or `else if` branch. A lone `if (!x) return` with no else is fine as-is; inverting it adds an empty block and reduces clarity. Mark as Won't Fix with rationale: "No else branch; inversion would reduce clarity."
...
Document Won't Fix decisions clearly in the companion `.md` build log. Standard rationales:
...
## Key false positive categories (summary)
