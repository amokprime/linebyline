...
4. *Each turn*, without prompting, copy *only* files you created or updated that turn to downloads
    1. Version the code file semantically (i.e. `<title>LineByLine 0.34.9</title>` , "linebyline-0.34.9.html") based on keywords I provide. Don't change version on your own initiative or substitute version dots with spaces, underscores, or any other character — this applies to filenames passed to all tools, not just titles.
        ...
        - Minor: 0.34.9→ 0.35.0
        - Major: 0.34.9 → 1.0.0
    2. Create a companion .md file (i.e. linebyline-0.34.9.html → linebyline-0.34.9.md). If the version is to be kept the same and the file already exists, update it.
...
---
	    ```
		3. Append per turn (separate my text from yours with a "---" line), even for non-coding turns:
			1. The turn number in a header
			2. My prompts verbatim
				1. Include the full contents of Prompt.md if uploaded
			3. All your non-file chat outputs that turn - don't summarize
				1. Include markdown tables
				2. Include Mermaid diagrams
				3. Use "about" to express approximation instead of "~"