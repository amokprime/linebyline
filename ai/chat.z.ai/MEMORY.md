...
- **S2083 path-traversal fix (split_bullets.py + lint_markdown.py, Sep 2026)** — SonarCloud's S2083 rule flags CLI scripts that pass `sys.argv` to `open()`/`Path.read_text()`/`Path.write_text()` without visible path validation.
    - The rule's taint analysis traces `sys.argv` → function calls → I/O, and requires the validation to be **in the same function body** as the I/O call (not in a helper function — taint analysis doesn't cross function boundaries).
    - **What didn't work:** (1) a `safe_path()` helper function called from `main()` — taint analysis couldn't trace the validation across the boundary; (2) `os.path.realpath` + `startswith(_BASE_DIR + os.sep)` using a module-level `_BASE_DIR` constant — SonarCloud didn't recognize the module-level variable as trusted.
    - **What worked:** inline `base_dir = os.path.realpath(os.getcwd())` directly in the I/O function, then `resolved = os.path.realpath(path_str)`, then `if resolved != base_dir and not resolved.startswith(base_dir + os.sep): raise ValueError(...)`, then `with open(resolved) as f: ...`. This matches SonarCloud's compliant solution verbatim — the base_dir derivation, the guard, and the I/O are all in the same function body, and `open()` receives the validated `resolved` path (not the original user input).
    - Key gotcha: SonarCloud's taint analysis recognizes `os.path.realpath` as a canonicalizer but needs the guard + I/O in the same scope. Module-level constants like `_BASE_DIR` are not recognized as "trusted" — the derivation must be inline.
