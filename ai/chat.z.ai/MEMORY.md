...
- **Tranche 5 — Sync/timestamp** — `useSync` composable + `src/utils/timestampSync.ts` (pure helpers) + `lrcParser.ts` additions (`hasLyricContent`/`hasTrailingTimestamp`/`allLyricLinesHaveTs`) + `EditorArea.vue` + `LeftPanel.vue` + `App.vue` wiring. 76 new specs (37 timestampSync + 12 lrcParser additions + 27 useSync). Full suite 307/307 green; `vue-tsc -b` clean.
    - `src/utils/timestampSync.ts` — pure helpers extracted from the monolith Sync/timestamp section. Ports the four unique-to-`tests/logic.spec.js` functions (`peelLastParen` from `_peelLastParen`, `findNextTimestampMs` from `_findNextTimestampMs`, `assignInterpolatedTs` from `_assignInterpolatedTs`, `batchSplitParens`) plus the internal helpers (`findPrevTsMs`/`findNextTsMs`/`findRunEnd` from `_findPrevTsMs`/`_findNextTsMs`/`_findRunEnd`, `findNextUnprocessedSplit`/`findNextNonMetaFromIdx` from `_findNextUnprocessedSplit`/`_findNextNonMetaFromIdx`). Naming convention drops the leading underscore (Phase C export convention matching `findLastMetaIdx`/`collapseBlanks`). The roadmap's Phase E consolidation begins here — these Vitest specs replace the corresponding `tests/logic.spec.js` sections once the Phase E cutover is verified.
