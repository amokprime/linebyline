...
description: Efficient section-targeted reading of linebyline-*.html to avoid loading the entire file unnecessarily. Use this skill at the start of every LineByLine coding session, and before reading any part of the app HTML. Contains the protocol for locating sections by name, reading only those sections, and patching the file with minimal token cost.
globs: ["docs/index.html"]
alwaysApply: false
---
...
If a patch shifts the section markers, update the reference list at the bottom of this skill with the new line numbers.

---

Section list (reference)

This list documents the section structure and contents. Actual line numbers must be found by grepping the file — this list is point-in-time, refreshed after patches that shift markers.
...
Helpers
  LRC parse          — TS_RE, META_RE, tsToMs, msToTs, isEndTs, replaceTs,
                       ...
                       _assignInterpolatedTs — reusable timestamp scans)
