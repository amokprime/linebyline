...
Run via the project `agent-tst` wrapper — see `.omp/RULES.md` for the wrapper's allowed arguments. Do not invoke `npx playwright` directly; the wrapper handles the SSH test infrastructure and master-key separation.

```sh
agent-tst -g <pattern>            # scoped run (preferred)
agent-tst tests/foo.spec.js       # single file
agent-tst --list                  # enumerate tests
agent-tst --project=chromium      # browser project override
```
