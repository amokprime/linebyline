...
// Hoisted regexes — avoids re-creating on each call + satisfies S8786/S6594.
const RE_DEFAULT_RE = /^\[re:\s*(.*)\]/m;
const RE_TAG_LINE = /^\[([a-zA-Z]+):\s*(.*)\]$/;
const RE_TAG_KEY = /^\[([a-zA-Z]+):/;
const RE_TAG_REPLACE = /^(\[re:\s*)(.*?)(\]\s*)$/m;
...
export function ensureReTagDefault(text: string, defaultMeta: string): string {
  const defReMatch = RE_DEFAULT_RE.exec(defaultMeta);
  const defReVal = defReMatch ? defReMatch[1]!.trim() : '';
  if (!defReVal) return text;
  return text.replace(RE_TAG_REPLACE, (match, pre, val, post) => {
...
export function mergeLrcMeta(lrcText: string, defaultMeta: string): string {
  ...
  lrcLines.forEach((l) => {
    ...
    if (!m) return;
    const key = m[1]!.toLowerCase();
    if (!defaultLines.some((dl) => RE_TAG_KEY.exec(dl)?.[1]?.toLowerCase() === key)) {
      extraFields.push(l);
    }
  });
